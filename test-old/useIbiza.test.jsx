...
describe('URL backed state', () => {
  ...
  it.skip('writes back to the server', async () => {
    function User() {
      const user = useIbiza('/user', { name: '!' })
      return (
        <>
          <div>{user.name}</div>
          <button onClick={() => void (user.name = 'Ash Moss')}>Update</button>
        </>
      )
    }
    const App = () => {
  ...
  it.only('can lazy fetch within a getter', async () => {
    const model = {
      foo: 'bar',
      get postAuthor() {
        console.log(this, this.isProxy)
    ...
    const spy = jest.spyOn(model, 'postAuthor', 'get')
    function Section() {
      const postAuthor = useIbiza('postAuthor', { name: 'lazy' })
      return <div>Name is {postAuthor.name}</div>
    }
    const App = () => {
      useIbiza(model)
      return (
